...
def signup(request):
    if request.method == "POST":
        ...
        # Mobile: convert safely, default to 0 if blank/invalid
        try:
...
def show_logs(request):
    ...
    # Get real Django logs
    logger = logging.getLogger('django')
    
    # Pull real app events from Message model
    try:
    ...
    try:
        ...
        for user in recent_users:
            logs_data.append({
                'time': now,
                'level': 'INFO',
                'message': f'User registered: {user.name} ({user.email})',
                'source': 'auth'
            })
    except Exception as e:
        logs_data.append({
    ...
    error_pattern = re.compile(r'(error|exception|traceback|failed|critical)', re.IGNORECASE)
    ...
    for line in reversed(lines[-500:]):
        ...
        if any(kw in line for kw in ['Watching for file changes', 'Apps ready', 'autoreload_started', 'Performing system checks', 'Starting development server']):
        ...
        if error_pattern.search(line):
            logs_data.append({'time': now, 'level': 'ERROR', 'message': line[:150], 'source': 'server'})
            continue
    ...
    try:
        logs_data.append({
            'time': now,
            'level': 'INFO',
            'message': f'Django server running - Total users: {User.objects.count()}, Total messages: {Message.objects.count()}',
            'source': 'system'
        })
    except Exception as e:
        logs_data.append({
